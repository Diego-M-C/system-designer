...
`1.1.0` · 2026-05-09 · **Feedback-layer closure release.** Closes the consumption-side seams identified by a focused 3-axis external audit on the feedback-learning layer. Headlines: (J-100) `templates/CLAUDE.md.tmpl` mandatory-reads now includes `feedback_learning/corrections.md` filtered to approved/incorporated → child agents read memorised corrections at session start (the user's anchor concern); (J-101) new **Phase 13.8 merge_verification** (`prompts/13_8_merge_verification.md`) closes the proposal → consensus → merge → **VERIFIED** → incorporated loop with cycle_trail.jsonl + INV-LIF-004 prior_hash chain; (J-102) consumption-gap wiring — `category=memory` corrections feed `memory_schema/manifest.json#evolution_log[]`; `category in {tooling,calibration,prompt_architect,portability}` recurring/systemic corrections auto-extend the AIE catalog; (J-103) corrections schema gains 4 cycle-trail join keys; (J-104) tracking template aligned with prompts (improvement_audit / feedback_learning / merge_verification / data_flow_validation / memory_schema / context_curation / adaptive_audit blocks); (J-105) dashboard `m.path` fallback resolves the v0.3.x KPI tiles + dynamic KPI count; (J-106) adaptive_audit_meta gains pre-action FTS5 recurrence guard (`prior_lessons[]`); (J-107) new T24 deterministic test guards never-default `learn_in_system` regression. Suite: 21 PASS / 0 FAIL.
`1.0.0` · 2026-05-09 · **Stable mature release.** Closes the entire external-audit jury checklist (P1 + P2 + P3 = 21/21 consolidated items). v1.0.0 ships the P3 batch on top of v0.4.0: adversarial-robustness rows under Art. 15(4) (model-extraction · adversarial-input · prompt-injection · supply-chain · 4 rows total); Art. 9(2) iterative-review-cadence rows (quarterly · post-incident · post-substantial-modification per Art. 43(4) · 3 rows); MiCA + PSD3 explicitly cross-walked in `references/eu_ai_act_mapping.md` for fintech; `references/memory_schema_protocol.md` + `<target_path>/memory_schema/manifest.json` added to `prompts/14_adaptive_audit_meta.md` `<dependencies>`; `cache_breakpoint_documented` rubric scoped to `Complex` tier only; wizard placeholder strings explicitly marked `is_template: true`. Total high-risk minimum audit rows now **123** (was 116 in v0.4.0 / 112 in v0.3.x). The system has cleared every consolidated finding from the original 3-axis blind audit panel.
`0.4.0` · 2026-05-09 · P2 batch from external-audit consensus jury (v0.3.2 deferred items). Headlines: tamper-evident sha256 hash-chain (`prior_hash`) on `observations.jsonl` + `decisions.md` (resuelve disagreement D1 — INV-LIF-004 + T23 deterministic test); Art.72 min_rows 6→10 to match AESIA Guide 13's four-sub-system enumeration; new wizard Q04c special-category-data → auto-emit GDPR Art.9 + Annex III(1) biometric rows; new `SystemSpec.hitl_mode` enum (in_the_loop / on_the_loop / over_the_loop / mixed) per AESIA Guide 6 §4.2.5; `gate_status` shape modernised to per-gate object covering 4.5 / 13.7 / 13.5; `public_sector/compliance_audit_trail` upgraded jsonl→sqlite; particular-tier `expected_entries` worked example + divide-by-zero edge in global tier; baseline-memory FS-existence check in prompt 15 verification.
`0.3.2` · 2026-05-09 · external-audit response release. Resolves the 9 P1 findings from a 3-axis blind audit panel (systems-architect / compliance / calibration-memory-feedback) consolidated by a consensus jury (`external_audit/jury_consensus.md`). Headline changes: (J-001) `non_removable_for_high_risk` symmetric across fintech/legal/public_sector starters; (J-002) Art.50 unbundled from Art.13 (sub-rows 6a/6b · Art.50 obligatory for limited-risk); (J-003) `SystemSpec.role` enum (provider | deployer | provider_and_deployer | distributor | importer) with role-aware row sub-selection in `06_eu_ai_act_mapper.md`; (J-004) Art.73 serious-incident-reporting workflow with 2/10/15-day deadline triggers + 3 HITL escalations; (J-005) `<selection_criteria>` and `<simulation_scenarios>` canonicalised in the prompt-architect taxonomy (now v1.1.0 / 124 tags); (J-006) T22 hardened to validate every top-level XML tag against the taxonomy; (J-007) T2 split into T2a (heuristic, non-blocking) + T2b (strict, exit-code-blocking); (J-008) dashboard refreshed to v0.3.2 with 18 phases + 9 new v0.3.x KPI cards; (J-009) doc-drift fixed across ARCHITECTURE.md / data-flow.md / README / orchestrator / starters. Plus J-018 typo fix (4 levels). Suite at 19 PASS / 0 FAIL.
`0.3.1` · 2026-05-08 · expands the memory-format taxonomy from 3 to **8** in `prompts/15_memory_schema_architect.md` and `references/memory_schema_protocol.md` — now covers `structured_md`, `csv`, `json`, `jsonl`, `sqlite`, `parquet`, `vector_db`, `graph_db`. Each module ALWAYS surfaces a recommended format + 2 calibrated alternatives (fit%) at HITL; the architect picks the best per workload (volume × query pattern × relationship density × audit-rule needs). Per-format portability tier + soft deps + fallback chain documented; audit-rule × format compatibility matrix added; anti-patterns explicitly refused.
